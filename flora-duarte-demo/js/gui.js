/* =========================================================
   ...
   quiz, buquê) e ganha a cena inteira na cena pós-créditos, em quatro atos
   dirigidos pelo scroll: paraquedas, notebook, cabo do WhatsApp e, no fim,
   entra na moldura e vira a foto de verdade.
...
(() => {
  ...
  let naCena = false;   // a pessoa está na cena pós-créditos
  ...
  const subir = (el, ms) => {
    // o recado do buquê não aparece por cima da cena pós-créditos
    if (!el || reduced || (naCena && el.id === 'guiBuque')) return;
  ...
  /* ---- ato 4: a foto em pixel ganha resolução enquanto ele entra nela ---- */
